...
description: The Apache CloudStack and Ubuntu/KVM IaaS Cloud Install Guide for x86_64 and ARM64
sidebar:
  order: 1
  badge: Popular
---
...
Ubuntu 24.04 (LTS) and KVM. If you're new to Apache CloudStack, here
a video for you:
...
:::note
This guide was updated for `ACS v4.22`. This may get outdated in future,
...
First install Ubuntu 24.04 LTS on your x86_64 system that has at least 8GB RAM
(prerably 16GB RAM or more) with Intel VT-X or AMD-V enabled. Ensure that the
...
:::note
Note the `-i` flag in the `cloudstack-setup-databases` command to override the IP address of the management server host if that's something different than what it detects automatically.
:::
...
On Ubuntu 22.04/24.04 or later, add `LIBVIRTD_ARGS="--listen"` to `/etc/default/libvirtd` instead:

```sh
    echo LIBVIRTD_ARGS=\"--listen\" >> /etc/default/libvirtd
```

For older Ubuntu versions, the traditional socket/listen based configuration may
not be supported out of the box. Run the following to get the old behaviour:
...
<script src="https://giscus.app/client.js"
        data-repo="rohityadavcloud/rohityadavcloud.github.io"
        data-repo-id="MDEwOlJlcG9zaXRvcnkzMDU5Nzk2"
        data-category="Comments"
        data-category-id="DIC_kwDOAC6wVM4Cwbtd"
        data-mapping="blog/cloudstack-kvm/"
        data-strict="0"
